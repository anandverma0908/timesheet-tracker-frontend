...
import { fetchFilters, downloadMonthlyReport, downloadFYReport } from '@/services/api'
...
const QUICK_DATES = [
  { label: 'This Month',   from: () => { const n = new Date(); return `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,'0')}-01` }, to: () => { const n = new Date(); const last = new Date(n.getFullYear(), n.getMonth()+1, 0).getDate(); return `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,'0')}-${last}` } },
  { label: 'Last Month',   from: () => { const n = new Date(new Date().getFullYear(), new Date().getMonth()-1, 1); return `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,'0')}-01` }, to: () => { const n = new Date(new Date().getFullYear(), new Date().getMonth(), 0); return `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,'0')}-${n.getDate()}` } },
  { label: 'This Quarter', from: () => { const m = Math.floor(new Date().getMonth()/3)*3; const n = new Date(); return `${n.getFullYear()}-${String(m+1).padStart(2,'0')}-01` }, to: () => { const m = Math.floor(new Date().getMonth()/3)*3+2; const n = new Date(); const last = new Date(n.getFullYear(), m+1, 0).getDate(); return `${n.getFullYear()}-${String(m+1).padStart(2,'0')}-${last}` } },
]
...
export default function ExportPage() {
  ...
  const [monthLabel, setMonthLabel]   = useState(() => toMonthLabel(filters.dateFrom ?? new Date().toISOString()))
  const [fyLabel, setFyLabel]         = useState('2025-2026')
  const [activeQuick, setActiveQuick] = useState(0)
  const [isDownloading, setIsDownloading] = useState(false)
  const [sheets, setSheets] = useState({
    rawData: true, podSummary: true, breakdown: true, pivot: false,
  })
  ...
  async function handleDownload() {
    ...
    const config: ExportConfig = {
      reportType, monthLabel, fyLabel,
      dateFrom, dateTo,
      pod, client, project, engineer,
      sheets,
    }
    try {
      if (reportType === 'monthly') await downloadMonthlyReport(config)
      else                          await downloadFYReport(config)
      toast.success(`${reportType === 'monthly' ? `timesheet_${monthLabel.replace(' ','_')}` : `engineering_FY_${fyLabel}`}.xlsx downloaded`)
    } catch (err) {
    ...
    <div className={styles.page}>
      ...
      <div className={`${styles.header} fade-up`}>
        <div>
          ...
          <p className={styles.subtitle}>Configure and download finance-ready Excel reports</p>
        </div>
      </div>
            ...
            <div className={styles.blockBody}>
              ...
              <div className={styles.formGrid}>
                <div className={styles.fg}>
                  ...
                  <input className="input" value={monthLabel} onChange={e => setMonthLabel(e.target.value)} />
                </div>
                <div className={styles.fg}>
                  <label className={styles.fl}>FY Label</label>
                  <input className="input" value={fyLabel} onChange={e => setFyLabel(e.target.value)} />
                </div>
                <div className={styles.fg}>
            ...
            <div className={styles.blockBody}>
              <div className={styles.sheetGrid}>
                {([
                  ...
                  >
                    <div className={styles.sheetCheck}>{sheets[s.key] ? '✓' : ''}</div>
                    {s.label}
                  </button>
                ))}
              </div>
            ...
            <div className={styles.blockBody}>
              <div className={styles.formGrid}>
                <div className={styles.fg}>
                  ...
                  <select className="select" value={project ?? ''} onChange={e => setProject(e.target.value || null)}>
                    <option value="">All Projects ({filtersData?.projects.length ?? 0})</option>
                    {filtersData?.projects.map(p => <option key={p}>{p}</option>)}
                  </select>
                </div>
                <div className={styles.fg}>
                  <label className={styles.fl}>POD</label>
                  <select className="select" value={pod ?? ''} onChange={e => setPod(e.target.value || null)}>
                ...
                <div className={styles.fg}>
                  ...
                  <select className="select" value={client ?? ''} onChange={e => setClient(e.target.value || null)}>
                    <option value="">All Clients ({filtersData?.clients.length ?? 0})</option>
                    {filtersData?.clients.map(c => <option key={c}>{c}</option>)}
                  </select>
                </div>
                <div className={styles.fg}>
                  <label className={styles.fl}>Engineer</label>
                  <select className="select" value={engineer ?? ''} onChange={e => setEngineer(e.target.value || null)}>
                    <option value="">All Engineers ({filtersData?.users.length ?? 0})</option>
        ...
        <div className={`${styles.preview} fade-up-2`}>
          <div className={styles.pvHead}>
            ...
            <div>
              ...
              <div className={styles.pvMeta}>
                {reportType === 'monthly' ? 'Monthly Finance' : 'Annual FY Engineering'} · {sheetCount} sheets
              </div>
            </div>
          </div>
          ...
          <div className={styles.pvStats}>
            <div className={styles.pvStat}>
              <div className={styles.pvStatVal} style={{ color: 'var(--accent)' }}>—</div>
              <div className={styles.pvStatLabel}>Hours</div>
            </div>
            <div className={styles.pvStat}>
              <div className={styles.pvStatVal} style={{ color: 'var(--green)' }}>—</div>
              <div className={styles.pvStatLabel}>Engineers</div>
            </div>
            <div className={styles.pvStat}>
              <div className={styles.pvStatVal} style={{ color: 'var(--purple)' }}>—</div>
          ...
          <div className={styles.pvFoot}>
            ...
            <button className={styles.dlBtn2} onClick={() => window.location.href = '/tickets'}>
              Preview in Table
            </button>
          </div>
        </div>
