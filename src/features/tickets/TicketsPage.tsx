      ...
      <div className={`${styles.header} fade-up`}>
        <div>
          ...
          <p className={styles.subtitle}>
            {isLoading
              ? "Loading…"
              : `${tickets.length.toLocaleString()} tickets · Virtualised`}
          </p>
        </div>
        {/* <div className={styles.actions}>
          ...
          <button
            className="btn btn-ghost btn-sm"
            onClick={() => toast.success("CSV downloading…")}
          >
            ↓ CSV
          </button>
          <button
            className="btn btn-primary btn-sm"
            onClick={() => toast.success("Excel downloading…")}
          >
            ↓ Excel
          </button>
        </div> */}
      ...
      <div className={`${styles.filterBar} fade-up-2`}>
        ...
        {activeFilters.map((f) => (
          <button
            ...
            onClick={() =>
              filters.setFilter(f.key as keyof typeof filters, null)
            }
          >
              ...
              {rowVirtualizer.getVirtualItems().map((vRow) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span className={`${styles.summary} truncate`}>
                        {t.summary}
                      </span>
                    </div>
                    <div
                      className={`${styles.td} ${styles.dim}`}
        ...
        {!isLoading && tickets.length > 0 && (
          <div className={styles.footer}>
            <span className={styles.footerInfo}>
              Showing {Math.min(tickets.length, 1847).toLocaleString()} of{" "}
              {(data?.count ?? 0).toLocaleString()} tickets
            </span>
          </div>
        )}
      </div>
